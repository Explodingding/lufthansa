...
def test_build_gold_layer_writes_business_ready_tables(tmp_path, spark) -> None:
    ...
    assert len(flight_performance) == 2
    ...
    assert gdn_fra["total_flights"] == 1
    assert gdn_fra["average_departure_delay_minutes"] == 12.0
    assert gdn_fra["delay_rate"] == 0.0

    passenger_communication = pd.read_parquet(gold_dir / "gold_passenger_communication")
    assert set(passenger_communication["event_type"]) == {
